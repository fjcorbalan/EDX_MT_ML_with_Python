...
def train(self, x1, x2, y):
    ...
    # Vectorising the provided scalar functions so they work on NumPy matrices
        vec_relu = np.vectorize(rectified_linear_unit)
        vec_relu_derivative = np.vectorize(rectified_linear_unit_derivative)
        vec_output_act = np.vectorize(output_layer_activation)
        vec_output_act_derivative = np.vectorize(output_layer_activation_derivative)
        ...
        activated_output = vec_output_act(output)

        # Backpropagation ###

        # Compute gradients
        output_layer_error = np.multiply((activated_output - y), vec_output_act_derivative(output))
        
        # Backpropagate error to hidden layer, factoring in the vectorized ReLU derivative
        hidden_layer_error = np.multiply(self.hidden_to_output_weights.T * output_layer_error, vec_relu_derivative(hidden_layer_weighted_input))
...
def predict(self, x1, x2):
    ...
    # Vectoring scalar functions so that they can handle NumPy matrices
    vec_relu = np.vectorize(rectified_linear_unit)
    vec_output_act = np.vectorize(output_layer_activation)
    ...
    activated_output = vec_output_act(output) # TODO
